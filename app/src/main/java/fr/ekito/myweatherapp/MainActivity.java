...
public class MainActivity extends AppCompatActivity implements MainActivityWeatherCallback {
   ...
   */
  @Override public void getWeatherData(final View view, final String location) {

    Snackbar.make(view, "Getting your weather :)", Snackbar.LENGTH_SHORT).show();

    WeatherSDK.getGeocode(location).map(new Func1<Geocode, Location>() {
      @Override public Location call(Geocode geocode) {
        return WeatherSDKUtil.INSTANCE.extractLocation(geocode);
      }
    }).switchMap(new Func1<Location, Observable<Weather>>() {
      @Override public Observable<Weather> call(Location location) {
        return WeatherSDK.getWeather(location.getLat(), location.getLng());
      }
    }).timeout(40, TimeUnit.SECONDS).subscribe(new Observer<Weather>() {
  ...
  private void setForecastForToday(DailyForecastModel dayX, TextView forecastDayX,
      IconTextView iconDayX) {
    forecastDayX.setText("Today : " + dayX.getForecast() + "\n" + dayX.getTemperatureString());
    iconDayX.setText(dayX.getIcon());
  }

  private void setForecastForDayX(DailyForecastModel dayX, int idx, TextView forecastDayX,
      IconTextView iconDayX, TextView tempDayX) {
